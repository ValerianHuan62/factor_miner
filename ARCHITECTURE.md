...
```mermaid
flowchart TD
    A[论文线索与经济假设] --> B[冻结 Gamma、测量和预算]
    B --> C[类型化 AST 与 FaVOR 构念一致性验证]
    C --> D[Quality：IC、稳定性与成本诊断]
    D --> E[Correlation/MFCF：数值冗余过滤]
    E --> F[SDF-inspired：联合重训与删除贡献]
    F --> G[SPPC-inspired：净 Sharpe Shapley 与代表库]
    G --> M[原始因子宽表与来源清单]
...
| 构念验证 | 冻结 `Gamma`、公式与当期可观测状态 | 五分位响应、覆盖和方向一致性 | 不读取收益标签；验证公式忠实性，不把它说成机制因果证明 |
| 质量与数值冗余 | 原始因子、冻结标签、共同截面 | IC/HAC/成本诊断、Pearson/Spearman、MFCF 聚类 | 相关过滤不等于独立预测信息 |
| 条件与经济冗余 | 相关过滤后的候选、冻结模型与交易政策 | 联合权重、逐因子删除重训 `ΔSharpe`、机制组 Shapley | 共同截面、标签事件 purge；普通 Ridge 不称 Ridge-SDF，组 Shapley 不称完整 SPPC |
